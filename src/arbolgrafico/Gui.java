...
public class Gui extends javax.swing.JFrame {
    ...
    Timer timer = new Timer(1000, new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
            if (index[0] < recorrido.size()) {
                simulador.limpiarNodosParaPintar();
                simulador.agregarNodoParaPintar(recorrido.get(index[0]));
                repintarArbol();
                index[0]++;
            } else {
    ...
    String recorridoTexto = null;
    List<Nodo> recorrido = null;

    if (opcion.equals("InOrden")) {
        recorridoTexto = this.simulador.inOrden().toString();
        recorrido = this.simulador.obtenerRecorridoInOrden();
    }
    if (opcion.equals("PreOrden")) {
        recorridoTexto = this.simulador.preOrden().toString();
        recorrido = this.simulador.obtenerRecorridoPreOrden();
    }
    if (opcion.equals("PosOrden")) {
        recorridoTexto = this.simulador.posOrden().toString();
        recorrido = this.simulador.obtenerRecorridoPosOrden();
    }
    if (opcion.equals("PorNiveles")) {
        recorridoTexto = this.simulador.imprimirPorNiveles().toString();
        recorrido = this.simulador.obtenerRecorridoPorNiveles();
    }
    if (opcion.equals("NivelOrdenado")) {
        recorridoTexto = this.simulador.porNivel().toString();
        
    }

    if (recorridoTexto != null) {
        this.impresion.setText(recorridoTexto);
    }
